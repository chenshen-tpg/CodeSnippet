package main.Coding.Algorithms.Medium.ExclusiveTime_636;
    ...
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for (String log : logs) {
            ...
            if ("start".equals(temp[1]))
                stack.add(new Pair<>(Integer.parseInt(temp[0]), Integer.parseInt(temp[2])));
            ...
            if (!stack.isEmpty() && Integer.parseInt(temp[0]) != stack.peek().getKey()) {
                Pair<Integer, Integer> current = stack.pop();
                stack.push(new Pair<>(current.getValue(), current.getValue() + Integer.parseInt(temp[2])));
            }
        }
        return res;
    }
